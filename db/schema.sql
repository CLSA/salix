...
-- Schema salix
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `salix` DEFAULT CHARACTER SET utf8mb4 ;
USE `cenozo` ;
...
CREATE TABLE IF NOT EXISTS `salix`.`apex_baseline_first_apex_exam` (
  `apex_baseline_id` INT(10) UNSIGNED NOT NULL,
  `apex_exam_id` INT(10) UNSIGNED NULL DEFAULT NULL,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
...
CREATE TABLE IF NOT EXISTS `salix`.`apex_deployment` (
  ...
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
  `apex_scan_id` INT(10) UNSIGNED NOT NULL,
  `apex_host_id` INT(10) UNSIGNED NOT NULL,
  `user_id` INT(10) UNSIGNED NULL DEFAULT NULL,
  ...
  CONSTRAINT `fk_apex_deployment_apex_scan_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_apex_deployment_user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `cenozo`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `salix`.`apex_deployment_code_summary` (
  `apex_deployment_id` INT(10) UNSIGNED NOT NULL,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
...
CREATE TABLE IF NOT EXISTS `salix`.`apex_exam` (
  ...
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
  `apex_baseline_id` INT(10) UNSIGNED NOT NULL,
  `serial_number_id` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `salix`.`apex_scan` (
  ...
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
  `apex_exam_id` INT(10) UNSIGNED NOT NULL,
  `scan_type_id` INT(10) UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `salix`.`custom_report`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `salix`.`custom_report` (
...
-- -----------------------------------------------------
-- Table `salix`.`role_has_custom_report`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `salix`.`role_has_custom_report` (
  `role_id` INT(10) UNSIGNED NOT NULL,
  ...
  CONSTRAINT `fk_role_has_custom_report_custom_report_id`
    ...
    REFERENCES `salix`.`custom_report` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
DELIMITER $$
USE `salix`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_apex_baseline_first_apex_exam`(IN proc_apex_baseline_id INT(10) UNSIGNED)
BEGIN
...
DELIMITER $$
USE `salix`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_apex_deployment_code_summary`(IN proc_apex_deployment_id INT(10) UNSIGNED)
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_baseline_AFTER_INSERT`
AFTER INSERT ON `salix`.`apex_baseline`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_baseline_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`apex_baseline`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_deployment_AFTER_INSERT`
AFTER INSERT ON `salix`.`apex_deployment`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_deployment_AFTER_UPDATE`
AFTER UPDATE ON `salix`.`apex_deployment`
FOR EACH ROW
BEGIN
  IF 0 = IFNULL( NEW.pass, 1 ) THEN
    UPDATE apex_scan SET priority = false WHERE id = NEW.apex_scan_id;
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_deployment_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`apex_deployment`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_exam_AFTER_DELETE`
AFTER DELETE ON `salix`.`apex_exam`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_exam_AFTER_INSERT`
AFTER INSERT ON `salix`.`apex_exam`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_exam_AFTER_UPDATE`
AFTER UPDATE ON `salix`.`apex_exam`
FOR EACH ROW
BEGIN
  CALL update_apex_baseline_first_apex_exam( NEW.apex_baseline_id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_exam_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`apex_exam`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_host_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`apex_host`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`apex_scan_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`apex_scan`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`code_AFTER_DELETE`
AFTER DELETE ON `salix`.`code`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`code_AFTER_INSERT`
AFTER INSERT ON `salix`.`code`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`code_AFTER_UPDATE`
AFTER UPDATE ON `salix`.`code`
FOR EACH ROW
BEGIN
  CALL update_apex_deployment_code_summary( NEW.apex_deployment_id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`code_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`code`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`code_type_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`code_type`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`scan_type_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`scan_type`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`scan_type_has_code_type_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`scan_type_has_code_type`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `salix`.`serial_number_BEFORE_INSERT`
BEFORE INSERT ON `salix`.`serial_number`
FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
