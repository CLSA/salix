...
-- Fri 20 Oct 2017 12:35:26 PM EDT
...
-- -----------------------------------------------------
-- Table `salix`.`apex_host_has_apex_scan`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `salix`.`apex_host_has_apex_scan` ;

CREATE TABLE IF NOT EXISTS `salix`.`apex_host_has_apex_scan` (
  ...
  `export_datetime` DATETIME NULL DEFAULT NULL COMMENT 'send to APEX host datetime',
  `import_datetime` DATETIME NULL DEFAULT NULL COMMENT 'receive from APEX host datetime',
  `note` TEXT NULL DEFAULT NULL,
...
USE `salix`$$
DROP TRIGGER IF EXISTS `salix`.`apex_host_has_apex_scan_BEFORE_INSERT` $$
USE `salix`$$
CREATE DEFINER = CURRENT_USER TRIGGER `salix`.`apex_host_has_apex_scan_BEFORE_INSERT` BEFORE INSERT ON `apex_host_has_apex_scan` FOR EACH ROW
BEGIN
SET NEW.create_timestamp = NOW();
END$$
