...
-- Thu 16 Jul 2020 01:48:13 PM EDT
...
USE `salix`$$
DROP TRIGGER IF EXISTS `salix`.`apex_deployment_AFTER_UPDATE` $$
USE `salix`$$
CREATE DEFINER = CURRENT_USER TRIGGER `salix`.`apex_deployment_AFTER_UPDATE` AFTER UPDATE ON `apex_deployment` FOR EACH ROW
BEGIN
  IF 0 = IFNULL( NEW.pass, 1 ) THEN
    UPDATE apex_scan SET priority = false WHERE id = NEW.apex_scan_id;
  END IF;
